...
| -- | 说明 | 图示 |
| -- | -- | -- |
| 单侧检验（上侧） | $H_0: \theta=\theta_0$ <br> $H_a: \theta>\theta_0$ <br> 拒绝阈：$Z > z_{\alpha}$ <br>  $p值=P(Z > z_c)=P(Z > 1.12)$ | ![](./Basis/8-1.png) |
| 单侧检验（下侧） | $H_0: \theta=\theta_0$ <br> $H_a: \theta<\theta_0$  拒绝阈：$Z < -z_{\alpha}$ <br>  $p值=P(Z < z_c)=P(Z < -1.12)$ <br> | ![](./Basis/8-2.png) |
| 双侧检验 | $H_0: \theta=\theta_0$ <br> $H_a: \theta \neq \theta_0$ <br> 拒绝阈：$ \|Z\| > z_{\alpha/2}$ <br>  $p值=2P(\|Z\| > z_c)=2P(\|Z\| > 1.12)$ | ![](./Basis/8-3.png) |
| -- | -- | 注：$Z=\frac{\hat{\theta}-\theta_0}{\sigma_{\hat{\theta}}}$ |
