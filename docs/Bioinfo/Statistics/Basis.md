...
| -- | -- | -- | -- | -- | -- |
| 中位数$\tau$ | $\tau=\tau_0$ | **上侧检验**：S=观测值中大于$\tau_0$的个数 <br> **双侧检验**：S=观测值中大于/小于$\tau_0$的个数（取二者间较大值） <br> **注**：忽略恰好等于$\tau_0$的个数，并相应的减小样本大小 n | -- | 检验单个总体的位置 |
|   |   | $Z=\frac{S-E(S)}{\sqrt{V(S)}}=\frac{S-0.5n}{\sqrt{(0.5)(0.5)n}}$ | 双侧检验的S可以视为二项分布中成功的个数；因此**大样本**($n \ge 10$)时，可以近似正态分布、利用Z统计量进行符号检验 | -- |
