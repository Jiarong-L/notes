...
* 贝叶斯法则：通过先验概率计算未知条件概率
    ...
    - 机器学习中，$P(c|x)=\frac{P(x,c)}{P(x)}=\frac{P(c)P(x|c)}{P(x)}$，判别式模型直接建模$P(c|x)$来预测c，生成式模型先建模$P(x,c)$、再由此获得$P(c|x)$、预测c
...
* **正态性检验**：见《Numerical Ecology》p190-193
    - [K-S test](https://zhuanlan.zhihu.com/p/292678346)（适合n>50），统计量：理论均值处CDF-最小数值处CDF
